...
def to_mesh(data):
    ...
    leaf_vec = np.zeros((leaf_num, 21, 3))
    ...
    return bark_wo_uv, leaf_wo_uv, bark, leaf


v_coor = [[394, 1015], [371, 909], [368, 852], [396, 720], [259, 722],
          [64, 561], [112, 527], [80, 277], [260, 348], [291, 191],
          [391, 45], [515, 184], [545, 326], [736, 264], [687, 540],
          [717, 589], [555, 744], [409, 722], [376, 838], [376, 881],
          [406, 1015]]
uv_corr = np.array(v_coor, dtype=np.float32)
uv_corr[:, 0] /= 800
uv_corr[:, 1] /= 1024

leaf_corr = np.copy(uv_corr)
leaf_corr[:, 0] = leaf_corr[:, 0] - 0.5
leaf_corr[:, 1] = 1 - leaf_corr[:, 1]
...
def dfs(fa_id, cur_id, level, xyz, is_leaf, leaf_size, vec, leaf_vec, id_to_leaf_id, tris, leaf_tris, uvs, leaf_uvs, tree):
    ...
    def add_tri(v1, v2, v3, id_book, uv_coor_book):
        leaf_tris.append([id_book[v1], id_book[v2], id_book[v3]])
        leaf_uvs.extend([uv_coor_book[v1], uv_coor_book[v2], uv_coor_book[v3]])

    def add_quad(v1, v2, v3, v4, id_book, uv_coor_book):
        add_tri(v1, v2, v3, id_book, uv_coor_book)
        add_tri(v1, v3, v4, id_book, uv_coor_book)
    ...
    if fa_id != -1:
        ...
        if is_leaf[cur_id] == 1:
            ...
            leaf_vec[leaf_id] = (leaf_pos.reshape(1, 3) + leaf_corr[:, 0].reshape(21, 1) * leaf_siz * dir_x.reshape(1, 3) +
                                                          leaf_corr[:, 1].reshape(21, 1) * leaf_siz * dir_z.reshape(1, 3))

            # v1 = leaf_id * 4 + 0
            # v2 = leaf_id * 4 + 1
            # v3 = leaf_id * 4 + 2
            # v4 = leaf_id * 4 + 3
            v_id = {"v%d" % (i+1): leaf_id*len(v_coor)+i for i in range(len(v_coor))}
            v_uv_coor = {"v%d" % (i+1): uv_corr[i].tolist() for i in range(len(v_coor))}

            add_quad("v1", "v2", "v20", "v21", v_id, v_uv_coor)
            add_quad("v2", "v3", "v19", "v20", v_id, v_uv_coor)
            add_quad("v3", "v4", "v18", "v19", v_id, v_uv_coor)
            add_quad("v5", "v6", "v7", "v4", v_id, v_uv_coor)
            add_quad("v7", "v8", "v9", "v4", v_id, v_uv_coor)
            add_quad("v9", "v10", "v11", "v4", v_id, v_uv_coor)
            add_quad("v18", "v11", "v12", "v13", v_id, v_uv_coor)
            add_quad("v18", "v13", "v14", "v15", v_id, v_uv_coor)
            add_quad("v18", "v15", "v16", "v17", v_id, v_uv_coor)
            add_tri("v4", "v11", "v18", v_id, v_uv_coor)
